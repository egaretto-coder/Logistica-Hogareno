...
const DB = {
  ...
  async loadAll(desdeISO) {
    if (!sb) return null;
    try {
      const [tarifas, superSla, panel, dim, km, kmTar, registros, config, rolPerm, roles, adelantos, adelantoCuotas, descItems] = await Promise.all([
